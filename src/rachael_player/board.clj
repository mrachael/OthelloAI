...
(defn get-moves
  ...
  (let [board (:board game-state)
    ...
    (valid-moves-for-player board color)))

(defn get-score-with-move
  [game-state move]
  (let [board (:board game-state)
        color (:color game-state)]
    (let [updated-board (update-board-for-move board move color)]
      (score updated-board color))))
